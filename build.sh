#!/bin/bash
# ============================================
# RUNIF OS 2.0 — Full Real Operating System
# Moscow Time (UTC+3) | Kernel Panic | Init
# ============================================
set -e
...
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}  RUNIF OS 2.0 Build System            ${NC}"
echo -e "${GREEN}========================================${NC}"

echo -e "${CYAN}[1/4] Setup...${NC}"
sudo apt update -qq 2>/dev/null || true
sudo apt install -y -qq nasm 2>/dev/null || true
rm -rf "$SYS" "$BUILD" "$ISO_DIR"
mkdir -p "$SYS/boot" "$SYS/kernel" "$BUILD" "$ISO_OUT"
[ -f "$DOWNLOADS/load_logo.png" ] && cp "$DOWNLOADS/load_logo.png" "$BUILD/load_logo.png"
[ -f "$DOWNLOADS/runif.png" ] && cp "$DOWNLOADS/runif.png" "$BUILD/runif.png"
echo "  OK"

# ============================================================
# BOOTLOADER
# ============================================================
cat > "$SYS/boot/bootloader.asm" << 'BOOTASM'
[ORG 0x7C00]
[BITS 16]
start:
    xor ax,ax
...
# KERNEL
# ============================================================
cat > "$SYS/kernel/kernel.asm" << 'KERNELASM'
[ORG 0x0000]
[BITS 16]
...
# BUILD
# ============================================================
echo -e "${CYAN}[2/4] Assembling...${NC}"
nasm -f bin "$SYS/boot/bootloader.asm" -o "$BUILD/bootloader.bin" && echo "  Bootloader OK" || exit 1
nasm -f bin "$SYS/kernel/kernel.asm" -o "$BUILD/kernel.bin" 2>&1 | grep -i error && exit 1 || echo "  Kernel OK"

KERNEL_SIZE=$(stat -c%s "$BUILD/kernel.bin")
echo "  Kernel: $KERNEL_SIZE bytes"

echo -e "${CYAN}[3/4] Creating image...${NC}"
SECTORS=$(( (KERNEL_SIZE + 511) / 512 ))
IMAGE_SECTORS=$(( SECTORS + 512 ))
dd if=/dev/zero of="$BUILD/disk.img" bs=512 count=$IMAGE_SECTORS 2>/dev/null
dd if="$BUILD/bootloader.bin" of="$BUILD/disk.img" conv=notrunc 2>/dev/null
dd if="$BUILD/kernel.bin" of="$BUILD/disk.img" bs=512 seek=1 conv=notrunc 2>/dev/null

LOGO_SECTOR=$(( SECTORS + 2 ))
[ -f "$BUILD/load_logo.png" ] && dd if="$BUILD/load_logo.png" of="$BUILD/disk.img" bs=512 seek=$LOGO_SECTOR conv=notrunc 2>/dev/null && echo "  Boot logo OK"
[ -f "$BUILD/runif.png" ] && dd if="$BUILD/runif.png" of="$BUILD/disk.img" bs=512 seek=$((LOGO_SECTOR+512)) conv=notrunc 2>/dev/null && echo "  Test image OK"

cp "$BUILD/disk.img" "$ISO_OUT/runif-os-2.0.img"

echo -e "${CYAN}[4/4] Creating run script...${NC}"
cat > "$ISO_OUT/run.sh" << 'RUNSH'
#!/bin/bash
qemu-system-i386 -fda "$HOME/runifkernel/iso/runif-os-2.0.img" -boot a -m 256M -rtc base=utc
RUNSH
chmod +x "$ISO_OUT/run.sh"

echo ""
echo "========================================"
echo "  BUILD COMPLETE"
echo "  Moscow Time (UTC+3) from CMOS RTC"
echo "  Init (PID 1) + Shell (PID 2)"
echo "  kill command -> KERNEL PANIC"
echo "  1,000,000 lines of panic (ENGLISH)"
echo "  Every 5 lines: press any key"
echo "  Real Snake | Real Pong | Real Calc"
echo "  Paged Help | Working Config"
echo "  Run: bash $ISO_OUT/run.sh"
echo "========================================"
